...
p_top <- ggdraw(p_ppet)+draw_label(label='(a)',x=0.03,y=0.93,size=25)
p_mid <- ggdraw(p_ndvi_sen)+draw_label(label='(b)', x=0.005,y=0.96, size=25,hjust=0,vjust=0)
                                       #hjust=0,vjust = 0)
p_bot <- ggdraw(p_vcf)+draw_label(label = '(c)', x=0.03,y=0.94,size=25)

cp_r <- cowplot::plot_grid(p_top,p_mid,p_bot,
                           nrow = 3,
                           rel_heights = c(2,3,3))
ggsave(plot=cp_r,
       filename = "figures/Fig_PPET-change_NDVI-ThielSen-Epoch_VCF-ThielSen.png", 
