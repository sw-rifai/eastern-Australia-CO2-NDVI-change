...
# cowplot produces odd grey lines
# library(cowplot)
# p_left <- ggdraw(p_annual)+draw_label(label='(a)', x=0.075,y=0.99,size = 14,fontface='bold')
# p_right <- ggdraw(p_ndvi)+draw_label(label='(b)', x=0.055,y=0.99,size = 14,fontface='bold')
# 
# p_joint <- cowplot::plot_grid(p_left,p_right,
#                            nrow = 1,ncol = 2,
#                            rel_widths = c(1,1.85), 
#                            align='h')
# dev.new(with=20, height=10, unit='cm')
# p_joint
# cowplot::save_plot(plot=p_joint,
#        filename = "figures/Fig3_ndvi-annual-rlm_ndvi-seasonal-TheilSen-trend-by-epoch.png",
#        base_width = 20, base_height=17, units='cm', dpi=350, 
#   device=grDevices::png)

p_out <- (p_annual|p_ndvi)+plot_annotation(tag_prefix = '(',tag_levels = 'a',tag_suffix = ')')+
  plot_layout()&theme(plot.margin = margin(t=0,r=0,b=0,l=0))
ggsave(p_out, 
  filename = "figures/Fig3_ndvi-annual-rlm_ndvi-seasonal-TheilSen-trend-by-epoch.png",
       width = 20, height=17, units='cm', dpi=350, 
  device=grDevices::png)

# p_joint <- p_annual|p_ndvi+plot_annotation(tag_prefix="(",
#   tag_levels='a',
#   tag_suffix=')')+plot_layout(heights=c(1,1.25))
# 
# ggsave(plot=p_joint,
#        filename = "figures/Fig3_ndvi-annual-rlm_ndvi-seasonal-TheilSen-trend-by-epoch.png",
#        width = 20, height=19, units='cm', dpi=350, 
#   device=grDevices::png)
