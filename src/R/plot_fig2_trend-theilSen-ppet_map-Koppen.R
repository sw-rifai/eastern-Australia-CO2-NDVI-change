...
p_kop <- kop[is.na(zone)==F] %>% 
  ...
  theme(#legend.position = c(0.75,0.85),
    ...
    legend.justification = c(1.0,1.00),
    ...
    plot.margin = margin(0,-10,0,-10)); p_kop
...
ggsave(cowplot::plot_grid(p_left,p_kop,ncol=2,
                          ...
                          rel_widths = c(1,1)),
       filename = "figures/Fig2_PPET-TheilSen_map-Koppen.png", 
...
p_out <- (p_left|p_kop+theme(plot.margin=margin(0,-10,0,-10)))+plot_annotation(tag_prefix = '(',tag_levels = 'a',tag_suffix = ')')&theme(plot.margin = margin(0,-10,0,-10))
ggsave(p_out,
       filename = "figures/Fig2_PPET-TheilSen_map-Koppen.png", 
       dpi=350, width=15, height=16,units='cm',type='cairo', 
  device=grDevices::png)
