---
title: "CO2-NDVI-effect_PETA_GAM_RLM"
output:
...
## Import data
```{r import_data, warning=FALSE, message=FALSE, cache=FALSE}
library(tidyverse)
...
dat <- dat[,`:=`(pe_12mo = precip_12mo/pet_12mo)]
dat <- merge(dat, 
             vi,
...
# Load LAI climatology 2002-2019 (before BS fires)
...
## Only pixel locations with more than 6 observations

In practice this is robust to the number of observations (6 - 18 as a minimum nob)
...
## Summarize NDVI and climate data to annual
...
## Regression: Long-term rate of NDVI increase per year 
...
g3_eval <- bam(ndvi_hyb~
                 te(mappet,co2,k=5)+                     # MAP:MAPET x CO2, with ~5 knots
                 te(mavpd15,frac_vpd_anom,k=5,bs='cs')+  # VPD x VPD_anom ... 
                 te(map,frac_p_anom,k=5,bs='cs')+        # MAP x P_anom ... 
                 te(mapet,frac_pet_anom,k=5,bs='cs')+    # MAPET x PET_anom ... 
                 epoch,
               data=dat_train, 
               discrete = T, # 
               select=TRUE, # turns on penalization term for splines
               method='fREML') # restricted maximum likelihood

```

### Out of sample R2 of GAM
...
## Regression: Robust linear model (RLM) CO2 + VPD + P + PET
...
## Regression Thiel Sen: VPD

```{r lt_v_sen, cache = TRUE}
system.time(
...
## Regression Thiel Sen: sqrt(VPD)

```{r lt_vsq_sen, cache = TRUE}
system.time(
...
## Construct relative increase of VPD in 2019

```{r}
# mavpd15 is annual mean from 1982:2011
...
## Percent increase of VPD, NDVI, Ca
...
# Expected WUE related increase using even split of WUE benefit (Donohue 2013)
...
## Relative change in VPD from 1982-2019
...
## WUE pred using 0.5 allocation to foliar

```{r, fig.height=6.5}
p_wue_sen <- lt_v_sen %>% 
  as_tibble() %>% 
  filter(b0 > 0) %>% # remove bad fits (don't really exist)
  filter(between(b1,-0.05,0.05)) %>% # remove bad fits (don't really exist)
  mutate(dVPD_VPD = b1*37/b0) %>% # relative fractional change in VPD
  mutate(expectation = 0.5*(dCa_Ca - 0.5*dVPD_VPD)) %>% # 0.5 alloc WUE pred
...
GAM estimated CO[2] contribution to NDVI where the effect of changes in P and PET are removed and the effect of increasing VPD is retained.
...
## Plot residuals between 0.5\*WUE and GAM
...
### Boxplots WUE preds. (0.25,0.5,0.75,1 & GAM estimate 
...
cp_r/p_bottom+patchwork::plot_layout(heights = c(1,0.5))
```

### Prepare PETA prediction comparison
...
dono17 <- {lt_vsq_sen %>% 
    ...
    # filter(between(b1,-0.05,0.05)) %>% 
    mutate(dVPD_VPD = b1*37/b0) %>% 
    mutate(wue_expectation = 0.5*(dCa_Ca - 0.5*dVPD_VPD)) %>% # 0.5*WUE benefit
    mutate(peta = (1+dCa_Ca)/(1+dVPD_VPD) -1) %>% # eq2 (Donohue 2017)
    # mutate(peta = (  (1+(dCa_Ca/341.16))/
    #                  (1+(dVPD_VPD/b0))) - 1) %>% 
    select(x,y,wue_expectation,peta)} %>% 
  inner_join(., lai,by=c('x','y')) %>% # join LAI
  mutate(alpha = 1-exp(-0.5*lai)) %>%  # eq 6
  mutate(beta = 1/(1+peta) - 1) %>% # eq 12
  mutate(dL = peta*(1-alpha)**2) %>%  # eq 9 
...
# boxplot
inner_join(dono17, gest) %>% 
  inner_join(., lest) %>% 
  inner_join(., dono17 %>% rename(dNDVI_dono17 = dL)) %>% 
  inner_join(., kop) %>%
...
```{r, fig.width=8}
p_peta <- gest %>% rename(GAM = dNDVI_co2) %>% 
  inner_join(., lest %>% rename(RLM=dNDVI_co2_rlm)) %>% 
  inner_join(., dono17 %>% rename(PETA = dL)) %>% 
  inner_join(., kop) %>%
  ...
  scale_color_manual(#option='D',end=0.6,direction = 1, 
    ...
    # drop=TRUE, 
    na.translate=FALSE)+
  labs(x=expression(paste('Mean Annual Leaf Area Index ',(m**2/m**2))),
       color='',
...
```
```{r}
p_dndvi <- inner_join(lt_ndvi_year %>% 
  ...
  geom_smooth()+
  geom_hline(lwd=1,col='grey50',aes(yintercept=0))+
  labs(x=expression(paste('Mean Annual Leaf Area Index ',(m**2/m**2))),
       color='N neighbors',
       y=expression(paste(Delta~'NDVI (%)')))+
...
```{r}
library(patchwork)
p_out <- (p_peta/p_dndvi)+plot_annotation(tag_levels = 'a', 
                                 tag_prefix = '(',
                                 tag_suffix = ')')
ggsave(p_out,
  filename = "figures/Fig_NDVI-CO2-effect-preds-xLAI_dNDVIxLAI.png",
       width = 15, height=20, units='cm', dpi=350, type='cairo')
